...
describe("TV Page", () => {
  ...
  it("renders loading state, watchlist tab, and switches to recently watched tab", async () => {
    ...
    const recentTabButton = screen.getByRole("tab", {
      name: /Recently Watched/i,
    });
  ...
  it("cleans up safely when unmounted before fetch resolves", async () => {
    ...
    (getWatchlistShows as jest.Mock).mockImplementation(
      () =>
        new Promise((res) => {
          resolveShows = res;
        }),
    );
    (getRecentEpisodes as jest.Mock).mockImplementation(
      () => new Promise(() => {}),
    );
