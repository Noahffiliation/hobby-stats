...
describe("Anime Page", () => {
  ...
  it("renders loading state, plan to watch tab, and switches to completed tab", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("tab", { name: /Plan to Watch \(325\)/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /Completed \(352\)/i }),
      ).toBeInTheDocument();
  ...
  it("handles empty planToWatch and completed arrays safely", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("No anime in plan to watch."),
      ).toBeInTheDocument();
    });
  ...
  it("logs error and displays error state when fetch fails", async () => {
    ...
    (getAnimeList as jest.Mock).mockRejectedValue(
      new Error("Anime fetch failed"),
    );
  ...
  it("cleans up safely when unmounted before fetch resolves", async () => {
    ...
    (getAnimeList as jest.Mock).mockImplementation(
      () =>
        new Promise((res) => {
          resolveAnime = res;
        }),
