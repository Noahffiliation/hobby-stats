...
describe("Games Page", () => {
  ...
  it("renders loading state, backlog tab, and switches to played tab", async () => {
    const mockData = {
      totalBacklog: 240,
      totalPlayed: 281,
      backlog: [{ title: "Silksong", platform: "PC" }, { title: "GTA VI" }],
      played: [
        {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("tab", { name: /Backlog \(240\)/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /Played \/ Reviews \(281\)/i }),
      ).toBeInTheDocument();
    ...
    const playedTabButton = screen.getByRole("tab", {
      name: /Played \/ Reviews/i,
    });
  ...
  it("handles empty backlog and played arrays safely", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("tab", { name: /Backlog \(0\)/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /Played \/ Reviews \(0\)/i }),
      ).toBeInTheDocument();
      expect(screen.getByText("No backlog games found.")).toBeInTheDocument();
    });

    const playedTabButton = screen.getByRole("tab", {
      name: /Played \/ Reviews \(0\)/i,
    });
    fireEvent.click(playedTabButton);
  ...
  it("logs error and displays error state when fetch fails", async () => {
    ...
    (getGamesList as jest.Mock).mockRejectedValue(
      new Error("Games fetch failed"),
    );
  ...
  it("cleans up safely when unmounted before fetch resolves", async () => {
    ...
    (getGamesList as jest.Mock).mockImplementation(
      () =>
        new Promise((res) => {
          resolveGames = res;
        }),
