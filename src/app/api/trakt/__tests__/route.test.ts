...
describe("Trakt API Route", () => {
  ...
  it("returns 400 for missing or invalid endpoint", async () => {
    ...
    const reqInvalid = new Request(
      "http://localhost/api/trakt?endpoint=invalid-ep",
    );
  ...
  it("fetches watched-shows endpoint with header count", async () => {
    (fetch as jest.Mock).mockResolvedValueOnce({
      ...
      headers: {
        get: (header: string) =>
          header === "X-Pagination-Item-Count" ? "45" : null,
      },
      json: async () => [{ title: "Show 1" }],
    });

    const req = new Request(
      "http://localhost/api/trakt?endpoint=watched-shows",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    const data = await res.json();
  ...
  it("fetches watchlist-movies with pagination support", async () => {
    (fetch as jest.Mock)
      .mockResolvedValueOnce({
        ...
        headers: {
          get: (header: string) =>
            header === "X-Pagination-Page-Count" ? "2" : null,
        },
    ...
    const req = new Request(
      "http://localhost/api/trakt?endpoint=watchlist-movies",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    const data = await res.json();
  ...
  it("fetches watchlist-shows single page", async () => {
    ...
    const req = new Request(
      "http://localhost/api/trakt?endpoint=watchlist-shows",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    const data = await res.json();
  ...
  it("fetches recent-movies successfully", async () => {
    ...
    const req = new Request(
      "http://localhost/api/trakt?endpoint=recent-movies",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    const data = await res.json();
  ...
  it("fetches recent-episodes successfully", async () => {
    ...
    const req = new Request(
      "http://localhost/api/trakt?endpoint=recent-episodes",
    );
    const res = await GET(req);
    expect(res.status).toBe(200);
    const data = await res.json();
