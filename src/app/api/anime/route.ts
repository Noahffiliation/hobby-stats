...
const COMPLETED_REGEX =
  /href="[^"]*status=2"[^>]*>[^<]*<\/a>\s*<span[^>]*>([0-9,]+)<\/span>/i;
const PTW_REGEX =
  /href="[^"]*status=6"[^>]*>[^<]*<\/a>\s*<span[^>]*>([0-9,]+)<\/span>/i;
...
export async function GET() {
  ...
  try {
    const [ptwRes, compRes, profileRes] = await Promise.all([
      fetch(
        `https://myanimelist.net/animelist/${username}/load.json?offset=0&status=6`,
        { headers },
      ),
      fetch(
    ...
    if (ptwRes.ok && compRes.ok) {
      const [ptwData, compData]: [RawAnimeItem[], RawAnimeItem[]] =
        await Promise.all([ptwRes.json(), compRes.json()]);
      ...
      if (profileRes.ok) {
        ...
        if (completedCountMatch) {
          totalCompleted = Number.parseInt(
            completedCountMatch[1].replaceAll(",", ""),
            10,
          );
        }
        if (ptwCountMatch) {
          totalPlanToWatch = Number.parseInt(
            ptwCountMatch[1].replaceAll(",", ""),
            10,
          );
        }
      }
