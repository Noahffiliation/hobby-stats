...
export default function Home() {
  ...
  const movieProgress =
    totalMovies === 0 ? 0 : (moviesWatched / totalMovies) * 100;
  const showProgress = totalShows === 0 ? 0 : (showsWatched / totalShows) * 100;
  const animeProgress =
    totalAnime === 0 ? 0 : (animeCompleted / totalAnime) * 100;
  const dramaProgress =
    totalDramas === 0 ? 0 : (dramasCompleted / totalDramas) * 100;
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 space-y-6 max-w-4xl mx-auto w-full my-auto text-center">
        <div className="text-center mb-2">
          <h1 className="text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
            Media Progress Dashboard
          </h1>
          <p className="mt-2 text-sm text-zinc-400">
            Tracking watched vs. watchlist progress across games, movies, TV
            series, anime, and K-dramas.
          </p>
        </div>
        ...
        {loading && (
          <div
            ...
            className="w-full max-w-2xl mx-auto py-12 text-center text-zinc-400 animate-pulse"
          >
            Loading stats...
          </div>
        )}
        ...
        {error && (
          <div
            ...
            className="w-full max-w-2xl mx-auto p-4 rounded-xl bg-red-950/40 border border-red-800/60 text-red-300 text-center text-sm"
          >
            {error}
          </div>
        )}
        ...
        {!loading && !error && (
          <div className="w-full max-w-2xl mx-auto space-y-8 bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800/80 backdrop-blur">
            <Progress
              label={`Game Progress - ${gamesPlayed} / ${totalGames}`}
              showValueLabel
              value={gameProgress}
            />
            <Progress
              label={`Movie Progress - ${moviesWatched} / ${totalMovies}`}
              showValueLabel
              value={movieProgress}
            />
            <Progress
              label={`Show Progress - ${showsWatched} / ${totalShows}`}
              showValueLabel
              value={showProgress}
            />
            <Progress
              label={`Anime Progress - ${animeCompleted} / ${totalAnime}`}
              showValueLabel
              value={animeProgress}
            />
            <Progress
              label={`K-Drama Progress - ${dramasCompleted} / ${totalDramas}`}
              showValueLabel
              value={dramaProgress}
            />
          </div>
