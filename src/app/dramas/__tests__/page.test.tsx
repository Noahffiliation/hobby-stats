...
describe("Dramas Page", () => {
  ...
  it("renders loading state, plan to watch tab, and switches to completed tab", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("tab", { name: /Plan to Watch \(237\)/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /Completed \(205\)/i }),
      ).toBeInTheDocument();
  ...
  it("handles empty planToWatch and completed arrays safely", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("tab", { name: /Plan to Watch \(0\)/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tab", { name: /Completed \(0\)/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByText("No K-dramas in plan to watch."),
      ).toBeInTheDocument();
    });

    const completedTabButton = screen.getByRole("tab", {
      name: /Completed \(0\)/i,
    });
    fireEvent.click(completedTabButton);
    expect(
      screen.getByText("No completed K-dramas found."),
    ).toBeInTheDocument();
  });

  it("logs error and displays error state when fetch fails", async () => {
    const consoleSpy = jest.spyOn(console, "log").mockImplementation();
    (getDramaList as jest.Mock).mockRejectedValue(
      new Error("Dramas fetch failed"),
    );
  ...
  it("cleans up safely when unmounted before fetch resolves", async () => {
    ...
    (getDramaList as jest.Mock).mockImplementation(
      () =>
        new Promise((res) => {
          resolveDramas = res;
        }),
