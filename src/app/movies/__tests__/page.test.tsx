...
describe("Movies Page", () => {
  ...
  it("renders loading state, watchlist tab, and switches to recently watched tab", async () => {
    ...
    const mockRecent = [
      {
        id: 1,
        movie: { title: "Recent Movie 1", year: 2023 },
        watched_at: "2026-08-01T12:00:00.000Z",
      },
      {
        id: 2,
        movie: { title: "Recent Movie 2" },
  ...
  it("cleans up safely when unmounted before fetch resolves", async () => {
    ...
    (getWatchlistMovies as jest.Mock).mockImplementation(
      () =>
        new Promise((res) => {
          resolveWatchlist = res;
        }),
    );
    (getRecentMovies as jest.Mock).mockImplementation(
      () => new Promise(() => {}),
    );
